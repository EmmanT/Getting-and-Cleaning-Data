...
dataresult1 <- data[,c(1,2,grep("std", colnames(data)), grep("mean", colnames(data)))]


# save dataresult1 into results folder
saveResult(dataresult1,"dataresult1")

## 5. Creates a second, independent tidy data set with the average of each variable for each activity and each subject. 
dataresult2 <- ddply(dataresult1, .(id, activity), .fun=function(x){ colMeans(x[,-c(1:2)]) })

# Adds "_mean" to colnames
colnames(dataresult2)[-c(1:2)] <- paste(colnames(dataresult2)[-c(1:2)], "_mean", sep="")

# Save tidy dataresult2 into results folder
saveResult(dataresult2,"dataresult2")
